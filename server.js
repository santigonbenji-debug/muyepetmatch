...
(function ensureIdsOnMascotas() {
  ...
  try {
    ...
    data.forEach((m, i) => {
      if (!m.id) { m.id = base + i; changed = true; }
      if (!m.createdAt) { m.createdAt = base + i; changed = true; }
    });
...
app.get("/inicio", (_, res) => res.sendFile(path.join(__dirname, "public", "inicio.html")));
app.get("/registro", (_, res) => res.sendFile(path.join(__dirname, "public", "registro.html")));
app.get("/login", (_, res) => res.sendFile(path.join(__dirname, "public", "login.html")));
app.get("/publicar", (_, res) => res.sendFile(path.join(__dirname, "public", "publicar.html")));
app.get("/mis-mascotas", (_, res) => res.sendFile(path.join(__dirname, "public", "mis-mascotas.html")));
app.get("/bandeja", (_, res) => res.sendFile(path.join(__dirname, "public", "bandeja.html")));
app.get("/mis-solicitudes", (_, res) => res.sendFile(path.join(__dirname, "public", "mis-solicitudes.html")));
app.get("/mascotas.json", (_, res) => res.sendFile(path.join(__dirname, "mascotas.json"))); // debug

/* ----------------- Uploads ----------------- */
function isImage(mt) { return ["image/jpeg", "image/png", "image/webp"].includes(mt); }
...
const uploadMascota = multer({
  ...
  fileFilter: (_req, file, cb) => !isImage(file.mimetype) ? cb(new Error("TIPO_NO_PERMITIDO")) : cb(null, true),
});
...
app.post("/api/login", loginLimiter, async (req,res)=>{
  try{
    ...
    if(!email || !password) return res.status(400).json({ok:false,error:"FALTAN_CAMPOS"});

    const usuarios = await readUsers();
    const u = usuarios.find(x=> normalizeEmail(x.email)===email);
    if(!u) return res.status(401).json({ok:false,error:"CREDENCIALES_INVALIDAS"});
    ...
    if(!ok) return res.status(401).json({ok:false,error:"CREDENCIALES_INVALIDAS"});
    return res.json({ok:true, usuario:{id:u.id, nombre:u.nombre, email:u.email}});
  }catch(e){
...
app.post("/api/registro", async (req, res) => {
  try {
    ...
    const alias     = String(req.body.alias     || "").trim().replace(/^@+/, "");
    ...
    return res.json({ ok: true, usuario: { id: nuevo.id, nombre: nuevo.nombre, email: nuevo.email } });
  } catch (e) {
...
app.post("/api/actualizar-usuario", (req, res) => {
  ...
  try {
    const usuarios = JSON.parse(fssync.readFileSync(USERS_PATH, "utf8") || "[]");
    const i = usuarios.findIndex((u) => normalizeEmail(u.email) === normalizeEmail(datos.email));
    if (i === -1) return res.status(404).json({ ok: false, error: "Usuario no encontrado" });
...
app.post("/api/usuario/foto", uploadPerfil.single("fotoPerfil"), async (req, res) => {
  try {
    const email = normalizeEmail(req.body.email);
    if (!email)  return res.status(400).json({ ok: false, error: "FALTA_EMAIL" });
    if (!req.file) return res.status(400).json({ ok: false, error: "FALTA_ARCHIVO" });
    const usuarios = await readUsers();
    const i = usuarios.findIndex((u) => normalizeEmail(u.email) === email);
    if (i === -1) return res.status(404).json({ ok: false, error: "USUARIO_NO_ENCONTRADO" });
...
app.post("/api/publicar", uploadMascota.single("foto"), (req, res) => {
  ...
  const archivo = path.join(__dirname, "mascotas.json");
  try {
    const arr = fssync.existsSync(archivo) ? JSON.parse(fssync.readFileSync(archivo, "utf8") || "[]") : [];
    arr.push(mascota);
...
app.post("/api/mascotas/:id/estado", (req, res) => {
  ...
  const archivo = path.join(__dirname, "mascotas.json");
  let mascotas = [];
  try { mascotas = JSON.parse(fssync.readFileSync(archivo, "utf8") || "[]"); } catch {}
  const i = mascotas.findIndex((m) => String(m.id) === String(id));
  if (i === -1) return res.status(404).json({ ok: false, error: "Mascota no encontrada" });
...
app.patch("/api/mascotas/:id", (req, res) => {
  const { id } = req.params;
  const allowed = [ "nombre", "edad", "tipo", "descripcion", "ubicacion", "foto" ];
  const cambios = {};
  allowed.forEach((k) => { if (req.body[k] !== undefined) cambios[k] = req.body[k]; });
  const archivo = path.join(__dirname, "mascotas.json");
  try {
...
app.post("/api/solicitudes", (req, res) => {
  ...
  if (!idMascota || !interesadoEmail)
    return res.status(400).json({ ok: false, error: "Falta idMascota o interesadoEmail" });

  const mp = path.join(__dirname, "mascotas.json");
  const sp = path.join(__dirname, "solicitudes.json");

  let mascotas = [];
  try { mascotas = JSON.parse(fssync.readFileSync(mp, "utf8") || "[]"); } catch {}
  const m = mascotas.find((x) => String(x.id) === String(idMascota));
  if (!m) return res.status(404).json({ ok: false, error: "Mascota no encontrada" });

  let actuales = [];
  try { actuales = JSON.parse(fssync.readFileSync(sp, "utf8") || "[]"); } catch {}
  const dup = actuales.some(
    (s) =>
      String(s.idMascota) === String(idMascota) &&
      String(s.interesadoEmail).toLowerCase() === String(interesadoEmail).toLowerCase() &&
      s.estado !== "rechazada",
  );
...
app.get("/api/solicitudes", (req, res) => {
  ...
  const sp = path.join(__dirname, "solicitudes.json");
  let arr = [];
  try { arr = JSON.parse(fssync.readFileSync(sp, "utf8") || "[]"); } catch {}
  if (duenioEmail)       arr = arr.filter((s) => s.duenioEmail === duenioEmail);
  if (interesadoEmail)   arr = arr.filter((s) => s.interesadoEmail === interesadoEmail);
  res.json(arr);
});
...
app.post("/api/solicitudes/estado", (req, res) => {
  ...
  if (!id || !estado) return res.status(400).json({ ok: false, error: "Faltan 'id' o 'estado'." });
  const i = arr.findIndex((s) => String(s.id) === String(id));
  if (i === -1) return res.status(404).json({ ok: false, error: "Solicitud no encontrada" });
  arr[i].estado = estado;
  if (estado === "aceptada") {
    arr[i].inter = {
...
app.post("/api/denuncias", (req,res)=>{
  try{
    ...
    if(!idMascota || !motivo) return res.status(400).json({ok:false,error:"Faltan datos"});
